...
ROS2D.OccupancyGrid = function (options) {
  ...
  for (var row = 0; row < this.height; row++) {
    for (var col = 0; col < this.width; col++) {
      ...
      // r
      imageData.data[i] = val + 26;
      // g
      imageData.data[++i] = val + 52;
      // b
      imageData.data[++i] = val + 100;
      // a
...
ROS2D.ChargeStation = function (options) {
  //	var that = this;
  options = options || {};
 ...
 * Internal use only
 */
ROS2D.ChargeStation.prototype.createPointShape = function (pos) {
  var point = new createjs.Shape();
  point.graphics.beginFill(this.pointColor);
  point.graphics.drawCircle(0, 0, 0.1, 0.1);
  point.x = pos.x;
  point.y = -pos.y;
 ...
 */
ROS2D.ChargeStation.prototype.addPoint = function (pos) {
 ...
 * Internal use only
 */
ROS2D.ChargeStation.prototype.drawFill = function () {
  var numPoints = this.pointContainer.getNumChildren();
  if (numPoints > 2) {
 ...
 */
ROS2D.ChargeStation.prototype.remPoint = function (obj) {
  var index;
  //	var point;
...
ROS2D.ChargeStation.prototype.__proto__ = createjs.Container.prototype;
...
ROS2D.PolygonMarker = function (options) {
  ...
  options = options || {};
  this.lineSize = options.lineSize || 3;
  this.lineColor = options.lineColor || createjs.Graphics.getRGB(0, 0, 255, 0);
...
ROS2D.Viewer = function (options) {
  ...
  setTimeout(() => {
    this.scene.children[2].lineColor = "rgba(255,0,255,0.3)";
  }, 1000);
